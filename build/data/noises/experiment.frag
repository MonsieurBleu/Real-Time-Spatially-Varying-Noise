...
void main()
{
    ...
    //     fragColor.rgb = smoothstep(vec3(0.0), vec3(1.0), fragColor.rgb);
    //     // fragColor.a = dmin3;
    ...
    {
        float hexSize = 0.1;
        vec2 hexDim = vec2(hexSize) 
            * vec2(1, SQR3)
            ;
        vec2 halfDim = hexDim*0.5;

        // vec2 cmin = round(auv/hexDim)*hexDim;
        // float dmin = distance(auv, cmin)/hexSize;

        // vec2 off = halfDim * (1.0 - 2.0*step(mod(auv, hexDim)/hexDim, vec2(0.5)));
        // vec2 auv2 = auv + off;
        // vec2 cmin2 = round(auv2/hexDim)*hexDim;
        // float dmin2 = distance(auv2, cmin2)/hexSize;

        // fragColor.rgb = 1.0 - min(dmin, dmin2).rrr;
        // fragColor.rgb = 1.0 - sqrt(min(D.x, D.y)).rrr;

        // fragColor.rgb = 1.0 - abs(dmin - dmin2).rrr;

        float xf = floor(auv.x/hexDim.x)*hexDim.x;
        float xc = ceil(auv.x/hexDim.x)*hexDim.x;

        float yf = floor(auv.y/hexDim.y)*hexDim.y;
        float yc = ceil(auv.y/hexDim.y)*hexDim.y;
        ...
        for(int i = 0; i < 4; i++)
        {
            d += clamp(1.0 - distance(auv, g[i])/hexDim.x, 0., 1.);
        }
        fragColor.rgb = d.rrr/2.0;
    }
}